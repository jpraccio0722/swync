...
//!   app writes `patterns.swync` and `swync-project.json` as you work.
//!
...
mod tests {
    ...
    fn a_file_appearing_moving_or_going_is_worth_reading_again() {
        ...
        assert!(matters(
            &event(EventKind::Create(CreateKind::File), &["/piece/kick.swync"]),
            root
        ));
        assert!(matters(
            &event(EventKind::Remove(RemoveKind::File), &["/piece/kick.swync"]),
            root
        ));
        assert!(matters(
            &event(
                EventKind::Modify(ModifyKind::Name(RenameMode::Both)),
                &["/piece/kick.swync", "/piece/lib/kick.swync"]
            ),
            root
        ));
        // What the platform could not classify. Read again: an extra listing
        // costs nothing and a missed file is the whole bug.
        assert!(matters(&event(EventKind::Any, &["/piece/kick.swync"]), root));
    ...
    fn a_burst_inside_a_hidden_folder_is_ignored() {
        ...
        assert!(matters(
            &event(
                ...
                &["/piece/.git/index.lock", "/piece/new.swync"]
            ),
            root
        ));
    }
    ...
    fn a_project_inside_a_hidden_folder_is_still_watched() {
        ...
        assert!(matters(
            &event(
                EventKind::Create(CreateKind::File),
                &["/home/someone/.local/piece/kick.swync"]
            ),
            root
        ));
        assert!(!matters(
    ...
    fn watching_nothing_ends_the_watch_that_was_running() {
        ...
        let root = std::env::temp_dir().join(format!("swync-watch-{}", std::process::id()));
    ...
    fn a_file_made_outside_the_app_reaches_the_editor() {
        ...
        std::fs::write(root.join("dropped-in.swync"), "let x = 1\n").expect("should write");
