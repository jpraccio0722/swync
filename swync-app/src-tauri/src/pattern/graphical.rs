...
//! is literal: the panel writes `patterns.swync` in the project folder, one
...
use crate::parser::parser::{parse, Expr, ListItem, SwyncItem};

/// What the panel's file is called, in every project.
pub const FILE: &str = "patterns.swync";
...
///
/// It is a generated file that people will open — it is swync, it sits in their
...
///
/// Which editor drew a row is not expressible in swync — it is not a property
/// of the pattern, it is a property of how someone chose to look at it. A
/// comment is where that belongs: the file stays entirely ordinary swync, the
...
/// is ordinary swync and somebody may well have written something in it that a
...
pub fn from_source(code: &str) -> Result<Vec<GraphicalPattern>, Diagnostic> {
    ...
    for item in &items {
        let SwyncItem::Let { name, value: Expr::List(steps) } = item else {
            continue;
        };
        let mut octave = None;
