...
impl Instruments {
    ...
    pub fn from_program(items: &[SwyncItem]) -> Instruments {
        Instruments {
            defs: items
                .iter()
                .filter(|i| matches!(i, SwyncItem::Function { .. }))
                .cloned()
                .collect(),
    ...
    pub fn param_count(&self, name: &str) -> Option<usize> {
        self.defs.iter().find_map(|i| match i {
            SwyncItem::Function { name: n, params, .. } if n.0 == name => Some(params.len()),
            _ => None,
        })
    }
}
...
pub fn build_voice(
    ...
    items.push(SwyncItem::Expr(Expr::Call {
